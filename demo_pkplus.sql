...
Demonstrate : 
 ...
 - Bonus, How to alter the table and fatten the pk: online !
...
create unique index OVL_PK_IND on OVL ( OWNER, TABLE_NAME );
alter table OVL add constraint OVL_PK primary key ( OWNER, TABLE_NAME ) using index ovl_pk_ind ;
...
alter table ovl drop constraint ovl_pk keep index ;
-- do not drop: drop index ovl_pk_ind yet ;

insert into ovl ( owner, table_name ) values ('SYS', 'DUAL' );

prompt Constraint dropped, but the UK is still here.
accept press_enter prompt "pk dropped, constraint still active..."

create index OVL_PK_fat_ind on ovl ( OWNER, TABLE_NAME, NUM_ROWS );
alter table OVL add constraint OVL_PK PRIMARY KEY ( OWNER, TABLE_NAME ) using index ovl_pk_fat_ind ;
-- now drop
drop index ovl_pk_ind ;

accept press_enter prompt "pk replaced,  enter to continue..."
