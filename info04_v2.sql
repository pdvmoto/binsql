...
set heading on
set feedback on
column tempfile_name        format a61 wrap
...
, dtf.status
, dtf.autoextensible
from cdb_temp_files dtf
order by 1
/


prompt Growth potential in files..


select df.file_name datafile_name
, round ( df.bytes      / (1024 * 1024) ) as mb 
, round ( df.user_bytes / (1024 * 1024) ) as user_mb
, round ( df.maxbytes   / (1024 * 1024) ) as max_mb
, round ( df.increment_by * 8196 / (1024 * 1024 ) ) as incr
-- , df.* 
from cdb_data_files df
union 
select df.file_name
...
*/
set doc on
col controlfile_name format a81 wrap
