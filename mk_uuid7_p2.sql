/**** /* 
  mk_uuid7_p2.sql: continuation.. experiments...
...
BEGIN
  ...
  ELSE
    ...
    dt_start_epoch := TO_DATE ( '1970-01-01', 'YYYY-MM-DD' ) ;   
    vc_fract       := TO_CHAR ( mod (  ( epoch_sec ), 1 ), 'FM.999') ; 

    IF vc_fract = '.' THEN 
      vc_fract := '.0' ;    -- generate a valid vc_fract
    END IF ;
 
    vc_interval := '0 00:00:00' || vc_fract ;

    -- dbms_output.put_line ( 'epoch_to_tmstmp :' || epoch_sec || ', vc_fract: ['|| vc_fract || ']' ) ; 
    -- dbms_output.put_line ( 'epoch_to_tmstmp : fract-interval = [' || vc_interval 
    --                          || ']  [' || to_dsinterval ( vc_interval ) || ']' )  ; 

    -- beware, if first term of sum is a DATE, the result will be DATE, and FF-interval is lost...
    -- hence CAST of the date (up to seconds) to a TIMESTAMP, then add interval
    the_timestamp  := CAST (  dt_start_epoch  
                            + ( trunc ( epoch_sec ) )  / ( 24*3600 ) AS TIMESTAMP )
                     + to_dsinterval ( vc_interval ) ; 

  END if ;

  -- dbms_output.put_line ( 'epoch_to_tmstmp:' || to_char ( the_timestamp, 'YYYY MM DD HH24:MI:SS.FF'  ) ) ;
...
BEGIN
  ...
  ELSE 
    ...
    -- dbms_output.put_line ( 'uuid7_epoch: uuid: ' || the_uuid || ', vc_hex = [' || vc_hex_epoch || ']' ) ;

    n_retval_epoch := to_number ( vc_hex_epoch, 'XXXXXXXXXXXX' ) / 1000  ;

  END IF ; 

  -- dbms_output.put_line ( 'uuid7_epoch: epoch = [' || n_retval_epoch || ']' ) ;
...
CREATE OR REPLACE FUNCTION uuid7_timestamp ( the_uuid RAW default null )
RETURN TIMESTAMP
IS
  ts_retval TIMESTAMP ;
BEGIN
  ts_retval := epoch_to_timestamp ( uuid7_epoch ( the_uuid ) ) ; 
  return ts_retval ; 
END;
/
list
...
CREATE OR REPLACE FUNCTION uuid7_date ( the_uuid RAW default null )
RETURN DATE
IS
  dt_retval DATE ;
BEGIN
  dt_retval := CAST ( epoch_to_timestamp ( uuid7_epoch ( the_uuid ) ) AS DATE )  ;
  return dt_retval ; 
END;
/
list
...
column ep_to_ts format A32

column the_day    format A12 
column the_count  format 999999 
...
prompt .
-- accept hit_enter prompt "check versions of generated uuids: v4, v7, sys-guid..."
host read         -t 10 -p "check versions of generated uuids: v4, v7, sys-guid..."  hit_enter 
prompt .
...
, to_char ( epoch_to_timestamp ( uuid7_epoch ( id ) ), 'YYYY MM DD HH24:MI:SS.FF' )   as ep_to_ts
from t7 order by id ; 
...
select  
  fmt_uuid ( id )     as fmt_uuid
, uuid7_epoch ( id )  as ts_epoch 
, to_char ( uuid7_timestamp ( ID  ), 'YYYY-MM-DD HH24:MI:SS.FF' )   as ep_to_ts
, to_char ( uuid7_date      ( ID  ), 'YYYY-MM-DD HH24:MI:SS' )   as ep_to_ts
from t7 order by id nulls first ; 


select  to_char ( uuid7_date ( id ), 'YYYY-MM-DD' ) as the_day
      , count (*)                                   as the_count
from tst_uuid7
group by to_char ( uuid7_date ( id ), 'YYYY-MM-DD' )
order by 1 ;
