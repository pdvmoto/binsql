...
column lfsync_tot     format 999,999.99  
column lfsync_avg     format   9,999.9   

column lfparw_tot    format 999,999.99
column lfparw_avg    format   9,999.9
...
--   AND  s2.begin_interval_time = ( SELECT MAX (begin_interval_time) FROM sys.wrm$_snapshot) -- is s2 the latest ?
  ...
  AND e1a.event_id        =  na.event_id
                                 AND na.event_name LIKE 'log file sync'
